...
import ch.epfl.scala.bsp4j.extended.GradleConfig;
...
public class GradleProjectConnector {
  ...
  /**
   * Build a Gradle connector.
   *
   * @param projectDir the project directory
   * @param config the Gradle configuration
   * @return the Gradle connector
   */
  ...
  private static void setConnectorConfig(
    ...
    if (!Strings.isNullOrEmpty(config.getUserHome())) {
      gradleConnector
      .useGradleUserHomeDir(buildGradleUserHomeFile(config.getUserHome(), projectDir));
    }
  ...
  /**
   * Get the system Gradle home directory.
   *
   * @return the system Gradle home directory
   */
